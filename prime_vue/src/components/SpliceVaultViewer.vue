<template>
  <div class="splice-vault-viewer">
    ...
    <div class="p-message p-message-warning mb-3">
    ...
    <!-- Loading progress indicator with percent -->
    <div v-if="loading" class="p-message p-message-info mb-3">
      <div class="p-message-wrapper">
        <span class="p-message-icon pi pi-spin pi-spinner"></span>
        <div class="p-message-text">
          Loading large dataset... This may take up to a minute.
        </div>
      </div>
    </div>
    ...
    <div class="p-grid p-align-center mb-3">
      ...
      <div class="p-col-12 p-md-2 flex align-items-end">
        <Button label="Search" icon="pi pi-search" class="w-full" @click="onSearch" :disabled="loading" />
      </div>
    </div>
  ...
  methods: {
    ...
    async fetchData(page = 1) {
      ...
      try {
        ...
        // Longer timeout for large dataset
        ...
        // Add timeout, validation options, and signal for abortion
        ...
        // Ignore abort errors (from user cancelling)
        ...
        if (error.response) {
          ...
          // Request timed out
          this.error = 'Request timed out. The large dataset is taking too long to process. Please try a more specific search.';
        } else {
